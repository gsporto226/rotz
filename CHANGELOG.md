...
- `rotz install` no longer aborts the whole run when a single install fails. A failed
  install is reported and its dependents are skipped, while unrelated installs continue.
  The command exits with a non-zero status if any install failed. The `--continue-on-error`
  flag has been removed since this is now the default behaviour.
