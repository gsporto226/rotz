...
## `shell_command` and environment propagation

When a dot is part of the dependency graph, rotz captures the environment produced by its
install command and propagates it to subsequent dependent installs. This relies on
detecting the shell from `shell_command`, and is therefore only supported for the default
shells `bash`, `zsh` and `powershell`/`pwsh`. For other shells (or when no shell is
configured) the install runs normally but without environment propagation.
